...
function createLoginPage() {
  ...
  secondaryButton.addEventListener('click', () => {
    ...
    confirmPasswordContainer.style.display = currentAuthMode === 'register' ? 'flex' : 'none';
    
    // Toggle password strength meter visibility
    passwordStrengthContainer.style.display = currentAuthMode === 'register' ? 'flex' : 'none';
    ...
    if (response.ok) {
      ...
      setTimeout(() => {
        ...
        if (authPopup) {
          ...
          const primaryButton = authPopup.container.querySelectorAll('button')[1]; // Second button
          const secondaryButton = authPopup.container.querySelectorAll('button')[0]; // First button
          ...
          // Hide confirm password field if it exists
          const confirmContainer = authPopup.container.querySelector('div[style*="display: flex"][style*="flex-direction: column"]:nth-child(3)');
          if (confirmContainer) {
            confirmContainer.style.display = 'none';
          }
          
          // Hide password strength if it exists
          const strengthContainer = authPopup.container.querySelector('div[style*="flex-direction: column"][style*="gap: 6px"]');
...
function addProfileButtonToSummary() {
  ...
  Object.assign(profileButtonContainer.style, {
    position: 'sticky',  // Changed to sticky to ensure always visible
    top: '5px',
    float: 'left',  // Left instead of right
    zIndex: 99999,
    marginTop: '5px',  // Adjust top margin
    marginLeft: '5px'  // Left margin instead of right
  });
