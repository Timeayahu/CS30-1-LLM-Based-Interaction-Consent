...
function init() {
  ...
  // Add periodic cache cleaning functionality
  setInterval(() => {
  ...
  // Initial scan of all links on the page
  setTimeout(() => {
...
// Modify existing initialization call
// setupLinkListeners();
init();
...
function handleLinkHover(e) {
  ...
  // Check if the link could be a privacy policy link
  if (isPossiblePrivacyPolicyLink(link)) {
    if (!floatingIcon) {
...
function checkLinkForPrivacyPolicy(link) {
  // Extended keyword regex, including multilingual keywords
  const privacyKeywords = /privacy|policy|privacy-policy|隐私|政策|隐私政策|条款|terms|personal|data|个人信息|个人资料|プライバシー|datenschutz|privacidad|cookie|cookies|쿠키|gdpr|ccpa|consent|同意|许可/i;
  ...
  // Performance optimization: First perform the simplest quick checks
  
  // 1. Check if URL contains privacy policy keywords
  if (privacyKeywords.test(link.href)) {
    return true;
  }
  
  // 2. Check if link text contains privacy policy keywords
  if (link.textContent && privacyKeywords.test(link.textContent)) {
    return true;
  }
  
  // 3. Check if link title attribute contains privacy policy keywords
  if (link.title && privacyKeywords.test(link.title)) {
    return true;
  }
  
  // 4. Check some common company privacy policy URL patterns (quick match)
  const knownPatterns = [
  ...
  // Performance optimization: Then perform more complex checks
  
  // 5. Check alt text of images inside the link
  const images = link.querySelectorAll('img');
  for (const img of images) {
    if (img.alt && privacyKeywords.test(img.alt)) {
      return true;
    }
  }
  
  // 6. Check link's aria label
  if (link.getAttribute('aria-label') && privacyKeywords.test(link.getAttribute('aria-label'))) {
    return true;
  }
  
  // 7. Check parent element's aria label (for nested buttons or wrapped links)
  if (link.parentElement && link.parentElement.getAttribute('aria-label') && 
      privacyKeywords.test(link.parentElement.getAttribute('aria-label'))) {
    return true;
  }
  
  // 8. Analyze link text match with common privacy policy link texts
  if (link.textContent && link.textContent.trim().length > 0) {
    const linkText = link.textContent.toLowerCase().trim();
    
    // Check if matches common privacy policy link texts
    for (const pattern of commonPrivacyLinkTexts) {
      if (pattern.test(linkText)) {
        return true;
      }
    }
    
    // Check if link text only contains few words (usually privacy links are short)
    // and surrounding nodes contain relevant terms (like "Legal", "About", etc.)
    if (linkText.split(/\s+/).length <= 3) {
      // Get text of surrounding sibling nodes
      const siblings = getSiblingLinkTexts(link, 3);
      
      // Check if sibling link texts contain relevant terms
      const legalContextWords = /legal|about|关于我们|法律|条款|联系/i;
      for (const siblingText of siblings) {
        if (legalContextWords.test(siblingText)) {
          // If there are legal-related links nearby, increase match probability
          return true;
        }
      }
    }
  ...
  // 9. Check if link is in footer area (common location for privacy policy links)
  if (isLinkInFooter(link)) {
    // If it's a footer link with short text, it might be a privacy policy
    if (link.textContent && link.textContent.trim().length < 30) {
      // Check if there are other legal/terms links nearby
      const siblings = getSiblingLinkTexts(link, 5);
      const legalSiblingCount = siblings.filter(text => 
        /legal|terms|cookie|about|contact|copyright|隐私|条款|版权|联系/i.test(text)
      ).length;
      
      // If there's at least one legal-related link nearby, probability increases
      if (legalSiblingCount > 0) {
        return true;
      }
    }
  }
  
  // 10. Check common redirection link patterns
  if (/redirect|goto|jumpto|r\.php|r\?|to=|link=|url=|rurl=|u=|ref=/i.test(link.href)) {
    // If it's a redirection link, check if the link text contains privacy policy keywords
    if (link.textContent && privacyKeywords.test(link.textContent)) {
      return true;
    }
  }
  
  // 11. Try decoding URL, check if it contains privacy keywords (for URL-encoded links)
  try {
    const decodedUrl = decodeURIComponent(link.href);
    if (decodedUrl !== link.href && privacyKeywords.test(decodedUrl)) {
      return true;
    }
  } catch (e) {
  ...
  if (!isLoading && !isError) {
    ...
    chrome.storage.local.get(['isAdmin'], async (result) => {
      ...
      try {
        ...
        const response = await fetch('http://localhost:8000/api/get_visibility?feature=extension');
