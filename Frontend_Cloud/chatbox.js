...
function parseMarkdown(text) {
  ...
  // Code blocks (triple backticks) - process first to avoid conflicts
  ...
  // Only wrap in <ol> if we have numbered list items that aren't already wrapped
...
function createChatWindow(buttonRect) {
  ...
  chatStyles.textContent = `
    ...
    /* Placeholder styles to ensure visibility in dark themes */
    #privacy-chat-window textarea::placeholder {
      color: #999 !important;
      opacity: 1 !important;
    }
    
    #privacy-chat-window textarea::-webkit-input-placeholder {
      color: #999 !important;
    }
    
    #privacy-chat-window textarea::-moz-placeholder {
      color: #999 !important;
      opacity: 1 !important;
    }
    
    #privacy-chat-window textarea:-ms-input-placeholder {
      color: #999 !important;
    }
  `;
...
function addChatMessage(text, sender) {
  ...
  if (sender === 'assistant' && text === 'Thinking...') {
    ...
    if (sender === 'assistant') {
      // Parse markdown and set as HTML for assistant messages
      const parsedHtml = parseMarkdown(text);
      messageDiv.innerHTML = parsedHtml;
      // Remove the pre-line white-space style for markdown content
...
function openChatWindowWithAutoQuery(buttonRect, bubbleData, buttonElement) {
  ...
  if (chatWindow) {
    ...
    setTimeout(() => {
      ...
      const autoMessage = `What is the detail explanation of ${bubbleData.keyword}?`;
...
function highlightImportantContent(messageElement) {
  ...
  // Check if the message already contains HTML elements (markdown formatted content)
  // If it has child elements other than just text nodes, skip highlighting to preserve markdown formatting
  const hasHtmlElements = messageElement.children.length > 0 || 
                         messageElement.innerHTML.includes('<') ||
                         messageElement.querySelector('code, pre, h1, h2, h3, strong, em, a, ul, ol, blockquote');
  
  if (hasHtmlElements) {
    // Message already has markdown formatting, skip highlighting to preserve it
    return;
  }
