...
function createLoginPage() {
  ...
  // Add CSS styles for input placeholders in dark themes
  ...
  // Add keyboard Enter key confirmation
  ...
  confirmPasswordInput.addEventListener('focus', () => {
    ...
    confirmPasswordInput.style.color = '#333';
  });
  ...
  // Add keyboard event listener to the entire login container
  loginContainer.addEventListener('keydown', (e) => {
    if (e.key === 'Enter' && !e.target.tagName.match(/input|textarea|select/i)) {
      e.preventDefault();
      primaryButton.click();
    }
  });
...
function createProfilePage(summaryRect) {
  ...
  if (isAdmin) {
    ...
    function initChatboxState() {
      ...
      fetch(`http://localhost:8000/api/get_visibility?feature=extension`)
    ...
    toggleSwitch.addEventListener('click', () => {
      ...
      fetch(`http://localhost:8000/api/toggle_visibility`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
