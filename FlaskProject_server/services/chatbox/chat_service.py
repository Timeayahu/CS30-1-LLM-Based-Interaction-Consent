...
class ChatService:
    ...
    def __init__(self):
        ...
        self.client, self.db, self.chat_sessions = connect_to_mongodb()
        
        # Verify privacy_data collection exists and has expected structure
        if self.db is not None:
            try:
                # Check if collection exists
                if 'privacy_data' not in self.db.list_collection_names():
                    self.logger.warning("privacy_data collection does not exist")
                else:
                    # Check if collection has documents
                    count = self.db['privacy_data'].count_documents({})
                    if count == 0:
                        self.logger.warning("privacy_data collection is empty")
                    else:
                        self.logger.info(f"privacy_data collection has {count} documents")
            except Exception as e:
                self.logger.error(f"Error checking privacy_data collection: {str(e)}")
    ...
    def close_connection(self):
        if self.client != None:
            close_mongodb_connection(self.client)
    ...
    def format_global_summary(self, policy_id):
        ...
        if policy_id:
            try:
                summary_data = get_summary(policy_id, self.db['privacy_data'])
    ...
    def get_policy_content(self, policy_id):
        ...
        try:
            policy_data = get_policy_by_id(policy_id, self.db['privacy_data'])
            if not policy_data:
...
I can help you understand privacy policies, data protection laws, user rights, and compliance matters."""
    ...
    def process_chat(self, data):
        ...
        try:
            ...
            session = get_session(self.chat_sessions, session_id) if session_id else None
            if not session:
                session_id = create_session(self.chat_sessions, policy_id=policy_id)
                self.logger.info(f"create new session: {session_id}")
                session = get_session(self.chat_sessions, session_id)
                if not session:
            ...
            if session.get("initialized", False):
                all_messages = get_session_messages(self.chat_sessions, session_id)
                if all_messages:
            ...
            if not session.get("initialized", False):
                mark_session_initialized(self.chat_sessions, session_id, "hierarchical_structure")
            
            # Save user message and AI reply to session
            add_message_to_session(self.chat_sessions, session_id, "user", f"Question: {user_question}")
            add_message_to_session(self.chat_sessions, session_id, "assistant", response_content)
